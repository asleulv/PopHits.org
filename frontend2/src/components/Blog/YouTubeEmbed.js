...
export default function YouTubeEmbed({ content }) {
  ...
  useEffect(() => {
    ...
    // Wrap all content in Sansita font with blog-content class
    let newContent = `<div class="blog-content font-sansita text-lg">${content}</div>`;
  ...
  return (
    <div 
      className="blog-content prose prose-xl max-w-none font-sansita prose-headings:font-bold prose-headings:text-gray-900 prose-p:text-gray-700 prose-a:text-pink-600 prose-a:no-underline hover:prose-a:text-pink-800 hover:prose-a:underline prose-img:rounded-lg prose-p:text-lg prose-li:text-lg"
