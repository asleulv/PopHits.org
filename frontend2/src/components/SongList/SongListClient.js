...
export default function SongListClient({
  ...
  const [onlyNumberOneHits, setOnlyNumberOneHits] = useState(initialNumberOneFilter || false);
  const [onlyUnratedSongs, setOnlyUnratedSongs] = useState(initialUnratedFilter || false);
  ...
  const updateUrl = () => {
    ...
    // Determine base path
    let path = '/songs';
    if (yearFilter) path = `/year/${yearFilter}`;
  ...
  const fetchData = async () => {
    try {
      ...
      if (error.message && error.message.includes('404')) {
        setError('No results found for this search. Please try different search terms.');
      } else {
        setError('An error occurred while fetching songs. Please try again later.');
      }
    ...
    <div>
      ...
      <div className="bg-white rounded-xl shadow-md p-4 mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            Peak Rank {sortField === 'peak_rank' && (sortOrder === 'asc' ? '↑' : '↓')}
          </button>
          <button
          ...
          >
            Score {sortField === 'average_user_score' && (sortOrder === 'asc' ? '↑' : '↓')}
          </button>
          <button
          ...
          >
            Weeks {sortField === 'weeks_on_chart' && (sortOrder === 'asc' ? '↑' : '↓')}
          </button>
        </div>
      </div>
      ...
      <div className="lg:hidden">
        {loading ? (
          ...
          <div className="space-y-4">
            {songs.map((song) => (
              <div key={song.id} className="bg-white rounded-xl shadow-md p-4 hover:shadow-lg transition-shadow">
                ...
                <div className="mt-3 flex justify-between text-sm">
                  <div className="flex space-x-4">
                    <div>
                      ...
                      <span className="font-medium ml-1">#{song.peak_rank}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Weeks:</span> 
                      <span className="font-medium ml-1">{song.weeks_on_chart}</span>
                    </div>
                  </div>
                  <div>
                    {song.average_user_score ? (
      ...
      <div className="mt-6 w-full text-center text-gray-600 bg-gradient-to-br from-gray-50 to-gray-100 p-3 rounded-lg shadow-sm">
        Showing <span className="font-bold text-pink-600">{showingFrom}-{showingTo}</span>{" "}
